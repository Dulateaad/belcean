...
export async function generateMetadata({ params: { locale } }: { params: { locale: Locale } }): Promise<Metadata> {
  ...
  const pageData = t.ServicePageGeneralnaya;
  return {
...
export default async function ServicePage({ params: { locale } }: { params: { locale: Locale } }) {
  ...
  const priceItem = prices.find(p => p.name.includes(t.Constants.services[2].title));
          ...
          <aside className="md:col-span-1 space-y-8 sticky top-24 self-start">
            <Card>
              ...
              <CardContent>
                  ...
                  <p className="text-sm text-muted-foreground mt-2">{t.ServicePages.price_postfix_complexity}</p>
              </CardContent>
            </Card>
            
            <Card className="bg-secondary">
               <CardHeader>
                  <CardTitle className="text-2xl">{t.ServicePages.form_title_general}</CardTitle>
               </CardHeader>
               <CardContent>
