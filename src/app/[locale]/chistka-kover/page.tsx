...
export async function generateMetadata({ params }: { params: Promise<{ locale: Locale }> }): Promise<Metadata> {
  ...
  const pageData = t.ServicePageKovrov;
  return {
...
export default async function ServicePage({ params }: { params: Promise<{ locale: Locale }> }) {
  ...
  const priceItem = prices.find((p: { id: string }) => p.id === 'carpet');
          ...
          <div className="md:col-span-2">
            ...
            <section id="process" className="mb-12">
              ...
              <div className="flex flex-col gap-6">
                   {pageData.work_process.map((step: { title: string; description: string }, index: number) => (
            ...
            <section id="faq">
              ...
              <Accordion type="single" collapsible className="w-full">
                {pageData.faq.map((item: { question: string; answer: string }, index: number) => (
          ...
          <aside className="md:col-span-1 space-y-8 sticky top-24 self-start">
            <Card>
              ...
              <CardContent>
                  ...
                  <p className="text-sm text-muted-foreground mt-2">{t.ServicePages.price_postfix_furniture}</p>
              </CardContent>
            </Card>
            
            <Card className="bg-secondary">
               <CardHeader>
                  <CardTitle className="text-2xl">{t.ServicePages.form_title_furniture}</CardTitle>
               </CardHeader>
               <CardContent>
